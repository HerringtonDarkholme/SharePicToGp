...
(function() {
  var BATCH_SIZE, BlobBuilder, GpAPI, IMGOBJ_MULTI, IMGOBJ_PIC_ARRAY_SIZE, IMGOBJ_SINGLE, SPAR_34_MULTI, SPAR_34_SINGLE, addListener, ajax, albumInfoLength, authuser, baseURL, canvasBlob, canvasPrototype, context, getStyleValue, hasArrayBuffer, hasArrayBufferViweSupport, hasBlob, imageInfoLength, imgList, init, messageHandler, openSession, postImage, rmvListener, setCallbacks, spar34Length, sparRequestLength, uploadImage, w, xhr_compatible,
  ...
  */
  ...
  postImage = function(postOption) {
    var albumID, albumInfo, albumUrl, buffer, c, callback, height, i, image, imgObj, imgObjPics, imgs, info, m, newNullArray, photoID, photoPageUrl, reqid, sessionID, spam, spar, tempArray, title, url, userID, width, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1,
      _this = this;
    ...
    */
    ...
    if ((this.album != null) && (this.album['albumName'] != null)) {
      ...
      spar[6] = JSON.stringify((function() {
        var _l, _len3, _ref2, _results;
        _ref2 = albumInfo.concat(imgs);
        _results = [];
        for (_l = 0, _len3 = _ref2.length; _l < _len3; _l++) {
          i = _ref2[_l];
          _results.push(JSON.stringify(i));
        }
        return _results;
      })());
      ...
      spar[6] = JSON.stringify((function() {
        ...
        _results = [];
        for (_m = 0, _len4 = imgs.length; _m < _len4; _m++) {
          i = imgs[_m];
          _results.push(JSON.stringify(i));
        }
        return _results;
  ...
  messageHandler = function(message, sender, sendResponse) {
    ...
    userBehavior = function(context) {
      ...
      getSelectedCircleNum = function() {
        return selectedCircles.querySelectorAll('div').length;
      };
      if (getSelectedCircleNum() > 0) {
        sendButton.className = 'active';
      }
      showCircleHandler = function(event) {
        ...
        return document[addListener]('click', hideCircleHandler);
      };
      hideCircleHandler = function(event) {
        event.stopPropagation();
        circleSlection.className = '';
        context.style.marginTop = "-" + (parseFloat(getStyleValue(context, 'height')) / 2) + "px";
        document[rmvListener]('click', hideCircleHandler);
        return circleArea[addListener]('click', showCircleHandler);
      };
      addCircleHandler = function(event) {
        var selected, self;
        event.stopPropagation();
        self = this;
        ...
        */
        ...
        selected[addListener]('click', function(event) {
          event.stopPropagation();
          selectedCircles.removeChild(selected);
          if (!(getSelectedCircleNum() > 0)) {
            sendButton.className = '';
          }
          return self[addListener]('click', addCircleHandler);
        });
        ...
        if (cvsB.imageSrc() === message['target']) {
          ...
          sendButton[addListener]('click', function() {
            ...
            if (sendButton.className === 'active') {
              ...
              return document[rmvListener]('click', hideCircleHandler);
            }
          });
          _results.push(cancelButton[addListener]('click', function() {
            document.body.removeChild(document.getElementById('Gpic-background'));
            return sendResponse();
          }));
        } else {
